...
export async function GET(req:Request){const actor=await getActorFromRequest(req);if(!actor)return unauthorized();if(!isOwner(actor))return forbidden();return NextResponse.json({tariffs:await prisma.tariff.findMany({orderBy:{percent:"asc"}}),currencies:await prisma.currency.findMany({orderBy:{code:"asc"}})});}
