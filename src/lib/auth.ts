...
export async function getActorFromRequest(req:Request):Promise<Actor|null>{const id=sessionUserId(cookieValue(req.headers.get("cookie")));if(!id)return null;const user=await prisma.user.findUnique({where:{id}});return user?{id:user.id,name:user.name,email:user.email,role:user.role}:null}
export async function getCurrentActor():Promise<Actor|null>{const id=sessionUserId(cookies().get(sessionCookie)?.value);if(!id)return null;const user=await prisma.user.findUnique({where:{id}});return user?{id:user.id,name:user.name,email:user.email,role:user.role}:null}
